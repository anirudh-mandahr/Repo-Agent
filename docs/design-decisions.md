...
*Known deviation.* The degraded-200 half of this policy holds when a specialist answers with an error or times out at the MCP layer. It does not yet hold when a specialist's **container is stopped**: the connect failure unwinds through the MCP client's anyio task group and raises `RuntimeError: Attempted to exit cancel scope in a different task than it was entered in`, which kills the orchestrator's request task without writing a response. The gateway then maps its own `chat_timeout_s` ceiling to HTTP 503, so the caller sees 503 after 90s rather than a degraded 200. Specialist and synthesis timeouts now use the cancellation-safe helper in `core/resilience/retry.py`, and the session pool no longer strands a slot per abandoned call, but that teardown path is still open. Tracked in the README limitations table and in [WALKTHROUGH.md](../WALKTHROUGH.md#failure-handling).
